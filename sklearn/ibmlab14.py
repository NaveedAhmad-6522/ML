...
np.random.seed(42)
noise_factores=0.5
X_noisy=X_scaled+noise_factores*np.random.normal(loc=0.0,scale=1,size=X.shape)
...
plt.subplot(1,2,1)
plt.hist(df[feature_names[5]],bins=20,color='blue',label="Original",alpha=0.7)
...
plt.subplot(1,2,2)
plt.hist(df_noisy[feature_names[5]],bins=20,color='red',label="Noisy",alpha=0.7)
plt.title('Noisy Feature Distribution')
plt.xlabel(feature_names[5])
plt.ylabel('Noisy Frequency')
